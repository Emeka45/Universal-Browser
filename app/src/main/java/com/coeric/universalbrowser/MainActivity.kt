...
class MainActivity : Activity() {
    ...
    private fun recoverSession(message: String) {
        runOnUiThread {
            ...
            try {
                session.open(getRuntime())
                if (target.isNotBlank()) session.loadUri(target)
                toast(message)
            } catch (_: Throwable) {
                showHome()
                toast("Browser recovered. Please try the page again.")
            }
        }
    }
    ...
    private fun buildHomePanel(): View {
        ...
        hero.addView(TextView(this).apply { text = "U"; textSize = 58f; typeface = Typeface.create("sans-serif", Typeface.BOLD); gravity = Gravity.CENTER; setTextColor(white); background = gradient(intArrayOf(violet, purple), 31.dp()); elevation = 7.dp().toFloat() }, LinearLayout.LayoutParams(104.dp(), 104.dp()).apply { bottomMargin = 17.dp() })
        hero.addView(TextView(this).apply { text = "UNIVERSAL"; textSize = 29f; letterSpacing = 0.08f; typeface = Typeface.create("sans-serif", Typeface.BOLD); setTextColor(white); gravity = Gravity.CENTER })
        ...
        hero.addView(EditText(this).apply { hint = "Search the web or enter a URL"; textSize = 15f; setSingleLine(true); setTextColor(ink); setHintTextColor(Color.rgb(125, 123, 140)); setPadding(18.dp(), 0, 18.dp(), 0); background = rounded(white, 19.dp()); elevation = 5.dp().toFloat(); setOnEditorActionListener { _, _, _ -> navigate(text.toString()); true } }, LinearLayout.LayoutParams(-1, 54.dp()))
    ...
    private fun featureCard(title: String, subtitle: String, mark: String, action: () -> Unit): View {
        ...
        row.addView(textBox); row.addView(TextView(this).apply { text = "›"; textSize = 25f; setTextColor(Color.rgb(150, 147, 166)) })
        return row
    }
    ...
    private fun renderExtensionManager(extensions: List<WebExtension>) {
        ...
        outer.addView(TextView(this).apply { text = "Manage the WebExtensions installed in Universal Browser."; textSize = 12.5f; setTextColor(muted); setPadding(0, 0, 0, 12.dp()) })
        if (extensions.isEmpty()) {
            ...
            empty.addView(TextView(this).apply { text = "Install a Firefox-compatible .xpi file or browse the add-ons store."; textSize = 12f; setTextColor(muted); gravity = Gravity.CENTER; setPadding(8.dp(), 5.dp(), 8.dp(), 12.dp()) })
            outer.addView(empty)
        } else {
        ...
        actions.addView(TextView(this).apply { text = "＋  Install extension from device"; textSize = 14f; typeface = Typeface.DEFAULT_BOLD; gravity = Gravity.CENTER_VERTICAL; setTextColor(white); setPadding(18.dp(), 0, 18.dp(), 0); background = gradient(intArrayOf(purple, violet), 16.dp()); setOnClickListener { openExtensionPicker() } }, LinearLayout.LayoutParams(-1, 50.dp()))
        actions.addView(TextView(this).apply { text = "Browse Firefox Add-ons"; textSize = 13f; typeface = Typeface.DEFAULT_BOLD; gravity = Gravity.CENTER; setTextColor(purple); setOnClickListener { navigate("https://addons.mozilla.org/android/") } }, LinearLayout.LayoutParams(-1, 45.dp()))
    ...
    private fun showAbout() {
        AlertDialog.Builder(this).setTitle("Universal Browser").setMessage("A lightweight Android browser built around GeckoView and WebExtensions.\n\nVersion 0.6.0\n\nDesigned for reliable everyday browsing on Android Go and low-memory devices.").setPositiveButton("Done", null).show()
    }
