...
/** Native media discovery bridge for playable, non-DRM media. */
object MediaBridge {
    ...
    private val delegate = object : WebExtension.MessageDelegate {
        override fun onMessage(nativeApp: String, message: Any, sender: WebExtension.MessageSender): GeckoResult<Any>? {
            if (nativeApp != NATIVE_APP || message !is JSONObject || message.optString("type") != "media_detected") return null
            val videos = message.optJSONArray("videos") ?: return null
            val activity = currentActivity ?: return null
            activity.runOnUiThread { showMediaChoices(activity, message.optString("pageTitle").ifBlank { "Universal media" }, message.optString("pageUrl"), videos) }
            return null
        }
    }
    ...
    private fun showMediaChoices(activity: Activity, title: String, referer: String, videos: JSONArray) {
        ...
        for (i in 0 until videos.length()) {
            ...
            if (url.isNotBlank() && !candidates.containsKey(url)) candidates[url] = MediaCandidate(url, item.optString("kind", "video"), item.optInt("width"), item.optInt("height"), item.optString("title").ifBlank { title })
        }
        if (candidates.isEmpty()) return
        val key = candidates.keys.sorted().joinToString("|")
        val now = System.currentTimeMillis()
        if (key == lastDialogKey && now - lastDialogAt < DIALOG_DEBOUNCE_MS) return
        lastDialogKey = key; lastDialogAt = now
        ...
        list.forEach { candidate ->
            val row = LinearLayout(activity).apply { gravity = Gravity.CENTER_VERTICAL; setPadding(12, 10, 8, 10) }
            ...
            val action = TextView(activity).apply { text = if (candidate.kind == "stream") "Qualities" else "Download"; textSize = 13f; setTextColor(0xFF6548FF.toInt()); setPadding(14, 8, 14, 8); setOnClickListener { if (candidate.kind == "stream" && candidate.url.substringBefore('?').lowercase(Locale.US).endsWith(".m3u8")) showStreamQualities(activity, candidate, referer) else startDownload(activity, candidate, referer) } }
            row.addView(label); row.addView(action); root.addView(row, LinearLayout.LayoutParams(-1, -2).apply { bottomMargin = 6 })
        }
    ...
    private fun showStreamQualities(activity: Activity, candidate: MediaCandidate, referer: String) {
        ...
        AdvancedMediaDownloadEngine.inspectHlsVariants(candidate.url, referer, onReady = { variants ->
            activity.runOnUiThread {
                if (activity.isFinishing || activity.isDestroyed) return@runOnUiThread
                if (variants.isEmpty()) { startDownload(activity, candidate, referer); return@runOnUiThread }
                val labels = variants.map { v -> "${if (v.height > 0) "${v.height}p" else "Auto"}${if (v.bandwidth > 0) " • ${v.bandwidth / 1000} kbps" else ""}" }.toTypedArray()
                android.app.AlertDialog.Builder(activity).setTitle("Choose video quality").setItems(labels) { _, which -> startDownload(activity, candidate.copy(url = variants[which].url), referer) }.setNegativeButton("Cancel", null).show()
            }
    ...
    private fun startDownload(activity: Activity, candidate: MediaCandidate, referer: String) {
        ...
        val extension = when { path.endsWith(".mp4") -> ".mp4"; path.endsWith(".webm") -> ".webm"; path.endsWith(".mov") -> ".mov"; path.endsWith(".m4v") -> ".m4v"; path.endsWith(".mp3") -> ".mp3"; path.endsWith(".m4a") -> ".m4a"; path.endsWith(".ogg") || path.endsWith(".oga") -> ".ogg"; else -> ".mp4" }
        val title = if (baseTitle.contains('.')) baseTitle else baseTitle + extension
        try {
            if (stream) AdvancedMediaDownloadEngine.enqueue(activity, url, title, referer, onStarted = { activity.runOnUiThread { toast(activity, "Media download started") } }, onFinished = { result -> activity.runOnUiThread { toast(activity, "Saved ${result.fileName}") } }, onError = { error -> activity.runOnUiThread { toast(activity, "Download failed: $error") } })
            else {
                ...
                DownloadService.start(activity, id, url, title, referer); toast(activity, "Download started")
            }
        } catch (error: Throwable) { toast(activity, "Download failed: ${error.message ?: "unknown error"}") }
    }

    private fun qualityLabel(candidate: MediaCandidate): String = "${if (candidate.height > 0) "${candidate.height}p" else if (candidate.width > 0) "${candidate.width}px" else "Auto"} • ${if (candidate.kind == "stream") "Stream" else "Video"}"
