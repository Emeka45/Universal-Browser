...
class DownloadService : Service() {
    companion object {
        ...
        fun start(context: Context, id: String, url: String, title: String, referer: String) {
            val intent = Intent(context, DownloadService::class.java).apply {
                action = ACTION_START
                putExtra(EXTRA_ID, id); putExtra("url", url); putExtra("title", title); putExtra("referer", referer)
            }
            if (Build.VERSION.SDK_INT >= 26) context.startForegroundService(intent) else context.startService(intent)
        }

        fun control(context: Context, action: String, id: String) {
            context.startService(Intent(context, DownloadService::class.java).apply { this.action = action; putExtra(EXTRA_ID, id) })
        }
    }
    ...
    override fun onCreate() {
        super.onCreate()
        createChannel()
        startForeground(NOTIFICATION_ID, notification("Preparing download", -1, null))
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> startTask(intent.getStringExtra(EXTRA_ID) ?: return START_NOT_STICKY)
            ACTION_RESUME -> startTask(intent.getStringExtra(EXTRA_ID) ?: return START_NOT_STICKY)
    ...
    private fun startTask(id: String) {
        ...
        val task = DownloadTaskStore(this).all().firstOrNull { it.id == id } ?: return
        stopRequested = false
        worker = Thread { download(task) }.also { it.start() }
    }

    private fun resumePersistedTask() {
        val task = DownloadTaskStore(this).all().firstOrNull { it.state == "downloading" || it.state == "queued" } ?: run { stopSelf(); return }
        startTask(task.id)
    }

    private fun pauseTask(id: String) {
        val task = DownloadTaskStore(this).all().firstOrNull { it.id == id } ?: return
        if (task.state == "downloading") DownloadTaskStore(this).updateState(id, "paused")
        stopRequested = true
        worker?.interrupt()
    }
    ...
    private fun cancelTask(id: String) {
        ...
        task?.tempPath?.let { File(it).delete() }
        store.update(id, state = "cancelled", bytes = 0L, tempPath = "")
        stopRequested = true
        worker?.interrupt()
    }
    ...
    private fun download(task: DownloadTaskStore.Task) {
        ...
        try {
            ...
            val connection = open(task.url, task.referer, existing)
            ...
            BufferedInputStream(connection.inputStream).use { input ->
                FileOutputStream(temp, append).use { output ->
                    ...
                    output.flush()
                    if (stopRequested) {
                        if (store.all().firstOrNull { it.id == task.id }?.state != "cancelled") store.updateBytes(task.id, bytes, total)
                        connection.disconnect(); return
                    }
                    store.updateBytes(task.id, bytes, if (total > 0) total else bytes)
                }
            }
            connection.disconnect()
            publishFile(temp, task)
            store.update(task.id, state = "completed", bytes = temp.length(), total = temp.length(), localUri = publishedUri.toString(), mimeType = guessMime(task.url), tempPath = "")
    ...
    private fun publishFile(temp: File, task: DownloadTaskStore.Task) {
        ...
        if (Build.VERSION.SDK_INT >= 29) {
            ...
            target.parentFile?.mkdirs(); temp.copyTo(target, overwrite = true)
    ...
    private fun createChannel() {
        if (Build.VERSION.SDK_INT >= 26) {
            (getSystemService(NOTIFICATION_SERVICE) as NotificationManager).createNotificationChannel(NotificationChannel(CHANNEL, "Downloads", NotificationManager.IMPORTANCE_LOW))
        }
    }
    ...
    private fun notification(title: String, bytes: Long, total: Long?, id: String?): Notification {
        ...
        if (total != null && total > 0) builder.setProgress(100, ((bytes * 100L) / total).toInt().coerceIn(0, 100), false)
        if (id != null) {
    ...
    private fun updateNotification(title: String, bytes: Long, total: Long, id: String, done: Boolean) {
        val notification = notification(title, bytes, if (total > 0) total else null, if (done) null else id)
