...
object BrowserExtensionCenter {
    ...
    fun showManager(activity: Activity, runtime: GeckoRuntime) {
        runtime.webExtensionController.list().accept({ extensions -> activity.runOnUiThread {
            ...
            if (list.isEmpty()) {
                AlertDialog.Builder(activity).setTitle("Extensions").setMessage("No extensions installed yet.\n\nUniversal Browser uses GeckoView WebExtensions. Firefox-compatible extensions are the best fit; Chrome-only APIs may not work.")
                    .setNegativeButton("Close", null).setPositiveButton("Install file") { _, _ -> openInstaller(activity) }.show()
                return@runOnUiThread
            }
            val labels = list.map { extensionLabel(it) }.toTypedArray()
            AlertDialog.Builder(activity).setTitle("Extensions (${list.size})").setItems(labels) { _, which -> showExtensionActions(activity, runtime, list[which]) }
                .setNeutralButton("Install file") { _, _ -> openInstaller(activity) }.setNegativeButton("Close", null).show()
    ...
    private fun showExtensionActions(activity: Activity, runtime: GeckoRuntime, extension: WebExtension) {
        ...
        AlertDialog.Builder(activity).setTitle(title).setMessage("${extension.metaData.description ?: "No description"}\n\nVersion: ${extension.metaData.version ?: "unknown"}\nID: ${extension.id}")
            .setItems(actions) { _, which ->
                when (which) {
                    0 -> { val result = if (enabled) runtime.webExtensionController.disable(extension) else runtime.webExtensionController.enable(extension); result.accept({ showManager(activity, runtime) }, { Toast.makeText(activity, "Extension action failed", Toast.LENGTH_SHORT).show() }) }
                    1 -> runtime.webExtensionController.setAllowedInPrivateBrowsing(extension, true).accept({ Toast.makeText(activity, "Private browsing permission updated", Toast.LENGTH_SHORT).show() }, { Toast.makeText(activity, "Could not update private permission", Toast.LENGTH_SHORT).show() })
                    2 -> AlertDialog.Builder(activity).setTitle("Remove extension?").setMessage("Remove $title from Universal Browser?").setNegativeButton("Cancel", null).setPositiveButton("Remove") { _, _ -> runtime.webExtensionController.uninstall(extension).accept({ showManager(activity, runtime) }, { Toast.makeText(activity, "Uninstall failed", Toast.LENGTH_SHORT).show() }) }.show()
    ...
    private fun install(activity: Activity, uri: Uri, runtime: GeckoRuntime) {
        try {
            ...
            AlertDialog.Builder(activity).setTitle("Install extension?").setMessage(details).setNegativeButton("Cancel", null).setPositiveButton("Install") { _, _ ->
                val controller = runtime.webExtensionController
                controller.install(Uri.fromFile(report.preparedFile).toString(), WebExtensionController.INSTALLATION_METHOD_FROM_FILE).accept({ extension -> activity.runOnUiThread {
                    if (extension != null) { controller.setAllowedInPrivateBrowsing(extension, false); Toast.makeText(activity, "${extension.metaData.name ?: report.name} installed", Toast.LENGTH_LONG).show() }
        ...
        } catch (error: Throwable) { Toast.makeText(activity, "Extension rejected: ${error.message ?: "invalid package"}", Toast.LENGTH_LONG).show() }
    }
}
