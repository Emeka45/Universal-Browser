...
/** Browser download center backed by Android DownloadManager, with progress and retry controls. */
object DownloadCenter {
    ...
    private fun row(activity: Activity, manager: DownloadManager, item: Item): LinearLayout {
        ...
        if (isActive(item.status)) {
            ...
            if (percent >= 0) box.addView(TextView(activity).apply { text = "$percent%"; textSize = 12f; gravity = Gravity.END; setTypeface(typeface, android.graphics.Typeface.BOLD) })
        }
        ...
        if (item.status == DownloadManager.STATUS_FAILED) {
            actions.addView(action(activity, "Retry") { retry(activity, manager, item) })
            actions.addView(action(activity, "Details") {
                AlertDialog.Builder(activity).setTitle("Download failed").setMessage(downloadReason(item.reason)).setPositiveButton("OK", null).show()
            })
        }
    ...
    private fun retry(activity: Activity, manager: DownloadManager, item: Item) {
        val url = item.url
        if (url.isNullOrBlank()) { Toast.makeText(activity, "Original URL is unavailable; cannot retry.", Toast.LENGTH_SHORT).show(); return }
        try {
            ...
            Toast.makeText(activity, "Download retry queued", Toast.LENGTH_SHORT).show()
            show(activity)
        } catch (error: Throwable) { Toast.makeText(activity, "Retry failed: ${error.message ?: "unknown error"}", Toast.LENGTH_SHORT).show() }
    }
    ...
    private fun query(manager: DownloadManager): List<Item> {
        val all = manager.query(DownloadManager.Query().setFilterByStatus(
            DownloadManager.STATUS_PENDING or DownloadManager.STATUS_RUNNING or DownloadManager.STATUS_PAUSED or DownloadManager.STATUS_SUCCESSFUL or DownloadManager.STATUS_FAILED
        ))
        all.use { c ->
            ...
            while (c.moveToNext()) {
                result += Item(
                    ...
                    status = c.getInt(status), reason = if (reason >= 0) c.getInt(reason) else 0,
                    downloaded = c.getLong(current), total = c.getLong(total),
    ...
    private fun progressPercent(item: Item) = if (item.total > 0L) ((item.downloaded.toDouble() / item.total) * 100.0).toInt().coerceIn(0, 100) else -1
    private fun statusText(item: Item): String {
        val state = when (item.status) {
            DownloadManager.STATUS_PENDING -> "Waiting"
            DownloadManager.STATUS_RUNNING -> "Downloading"
            DownloadManager.STATUS_PAUSED -> "Paused (Android will resume when possible)"
    ...
    private fun formatBytes(bytes: Long): String {
        ...
        val units = arrayOf("KB", "MB", "GB", "TB"); var value = bytes.toDouble(); var index = -1
        while (value >= 1024.0 && index < units.lastIndex) { value /= 1024.0; index++ }
        return String.format(Locale.US, "%.1f %s", value, units[index])
    }
    private fun downloadReason(reason: Int) = when (reason) {
